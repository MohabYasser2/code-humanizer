...
- 🟡 **CONDITIONAL** — behavior-neutral *only if* applied completely and in a compatible
  ...
  for this type). **Apply only after verifying the precondition; otherwise treat as 🔴.**
...
> **The hard rule that keeps code working:** the skill auto-edits 🟢 and *verified* 🟡 only.
> 🔴 is reported, never written. If you cannot prove a 🟡 edit is total and safe, demote it
> to 🔴 and flag it. When unsure, flag — don't guess.
...
### Verifying a 🟡 rename — the complete-reference rule

Renames (H27–H31) are the most common 🟡 edit and the easiest to get wrong: a rename is
...
5. **Scope discipline:** only rename **locals and single-file private symbols** as 🟡. A
   field, property, public method, or exported symbol is **🔴 — flag, never auto-rename** —
   because step 3's surface is too large to clear by inspection.

The same rule covers any 🟡 that depends on completeness (e.g. a file rename, H50): incomplete =
broken, so verify totality or demote to 🔴.
...
|---|---|---|---|
| Operator spacing `a=b` vs `a = b` | 🟢 (PEP8 warn, runs) | 🟢 | 🟢 (gofmt reverts on save) |
| Space before `;` / inside `( )` | n/a | 🟢 | 🟢 |
| Indent **width** change (2↔4) | 🔴 **IndentationError** | 🟢 | 🟢 (tabs) |
| Mixed tabs/spaces | 🔴 **TabError** | 🟢 | 🔴 (tabs required) |
| Brace style `if(x){` vs `if (x) {` | n/a | 🟢 | 🟢→gofmt |
| Blank lines / trailing whitespace | 🟢 | 🟢 | 🟢→gofmt |
| `#`/`//` comment spacing | 🟢 | 🟢 | 🟢 |

**Rule:** in Python, *never* touch leading indentation or tabs (🔴). All Python whitespace
...
🟢 because the compiler ignores it.

> The headline example `int x=y ;` (space before semicolon, no spaces around `=`) is a
> brace-language signal — 🟢 in C/C++/Java/C#/JS/TS, not applicable in Python.
...
🟡; JS `===`→`==` and `let`→`var` are 🔴; C# `var`↔explicit and `#region` are 🟢) and the
...
|----|--------|---------|------|
| H1 | Inconsistent operator spacing | `x=y` here, `x = y` there | 🟢 brace · 🟢 Py |
| H2 | Space before `;` / inside parens | `int x = y ;` · `foo(a ,b)` | 🟢 brace |
| H3 | Trailing whitespace on some lines | (invisible; detectors measure it) | 🟢 |
| H4 | Irregular blank lines | 3 blanks, then 0 between the next pair | 🟢 |
| H5 | Inconsistent indent **width** | 4 spaces here, 2 there | 🟢 brace · 🔴 **Py** |
| H6 | Mixed tabs/spaces | | 🟢 some · 🔴 **Py/Go** |
| H7 | Variable line length | 30-char lines next to 140-char ones | 🟢 |
| H8 | Align `=` in one block, abandon in next | | 🟢 |
| H9 | Mixed quote style | `'foo'` and `"bar"` for no reason | 🟢 (watch escaping) |
| H10 | Mixed brace/keyword spacing | `if(x)` and `if (x)` in one file | 🟢 brace |
| H11 | No-space-after-`#`/`//` | `#like this` mixed with `# like this` | 🟢 |

### Comments — the human kind (AI strips these; their presence is the strongest tell)

| ID | Signal | Example | Tier |
|----|--------|---------|------|
| H12 | **Commented-out code left in** | `# old_lr = 3e-4` · `// for(i..) print(i)` | 🟢 |
| H13 | Debug prints | `print("here")` `print("aaaa")` · commented `# breakpoint()` | 🟢 commented · 🟡 live (adds a side effect) |
| H14 | TODO/FIXME/HACK/XXX with real context | `# FIXME: breaks on empty input, ask Omar` | 🟢 |
| H15 | Stale comment that contradicts the code | comment says "0=stay" but `0` means up | 🟢 |
| H16 | Venting / emotional | `# this is dumb but it works` · `# don't touch` | 🟢 |
| H17 | Dated / signed scribbles | `# 12/03 hack — revisit` · `# -MY` | 🟢 |
| H18 | Native-language comment | a non-English line mid-file | 🟢 |
| H19 | Pragmatic suppressions | `# noqa` `# type: ignore` `# pylint: disable=...` | 🟡 (must be a real, valid pragma) |
| H20 | External references | `# from stackoverflow.com/q/...` · ticket #, a name | 🟢 |
| H21 | Half-finished thought | `# need to handle the case where` (stops) | 🟢 |

### Spelling, typos, grammar

| ID | Signal | Example | Tier |
|----|--------|---------|------|
| H22 | Typos in **comments / log strings** | `recieve`, `seperate`, `occured`, `lenght` | 🟢 |
| H23 | Typos in **identifiers** | `recieved_data`, `widht` used everywhere | 🔴 **flag-only** (one missed ref = `NameError`) |
| H24 | British/American mix | `colour`/`color` in comments | 🟢 comment · 🔴 identifier |
| H25 | Non-native / loose grammar in comments | "this make the thing for calc" | 🟢 |
| H26 | Inconsistent message capitalization | `"Error:"` vs `"error :"` | 🟢 (watch exact-match consumers) |

### Naming chaos

| ID | Signal | Example | Tier |
|----|--------|---------|------|
| H27 | Mixed conventions in one file | `camelCase` next to `snake_case` | 🟡 (rename all refs) |
| H28 | Numbered / lazy locals | `tmp`, `tmp2`, `data2`, `final`, `real_final` | 🟡 |
| H29 | Copy-paste residue | `df_copy`, `user2`, `result_old` | 🟡 |
| H30 | Idiosyncratic abbreviations | `val`, `cfg`, `idx`, `n`, `foo` | 🟡 |
| H31 | Inconsistent plurality | `user` holds a list; `items` holds one | 🟡 |

### Structural / pragmatic scars

| ID | Signal | Example | Tier |
|----|--------|---------|------|
| H32 | Copy-paste duplication (WET) | same 6 lines pasted 3× instead of a loop | 🟢 (more code, same behavior) |
| H33 | Magic number inline | `if speed > 0.47:` no named constant | 🟢 |
| H34 | Hardcoded personal path | `C:\Users\mohab\...` `/home/omar/` | 🟢 (never inject real secrets) |
| H35 | One oversized function (don't split) | 150-line `main()` | 🟢 (leave as-is) |
| H36 | Dead / unreachable branch | `if False:` block, code after `return` | 🟡 (must stay unreachable) |
| H37 | `i = i + 1` instead of `i += 1` (mixed) | | 🟢 |
| H38 | Leftover debug scaffolding | `t0 = time.time()` timing, `import pdb` | 🟡 (side effects/imports) |
| H39 | Inconsistent error handling | careful `try` here, none there | 🟡 |
| H40 | "Temporary" hack made permanent | `time.sleep(2)  # why does this fix it??` | 🔴 (sleep changes timing) |

### Import habits

| ID | Signal | Example | Tier |
|----|--------|---------|------|
| H41 | Import added mid-file at point of use | `import os` on line 80 | 🟢 |
| H42 | Unused import left after a refactor | imported `json`, never used | 🟢 |
| H43 | Chronological (not alphabetical) order | order they were needed | 🟢 |
| H44 | Commented-out import | `# import seaborn as sns` | 🟢 |

### Legacy / non-idiomatic syntax

| ID | Signal | Example | Tier |
|----|--------|---------|------|
| H45 | `== True` / `== None` | `if active == True:` `if x == None:` | 🔴 **flag-only** — semantics differ for numpy/None |
| H46 | `range(len(x))` instead of `enumerate` | | 🟢 |
| H47 | `len(x) == 0` instead of `not x` | | 🟡 (differs for numpy/pandas) |
| H48 | Redundant parens | `return (x)` `if (a and b):` | 🟢 |
| H49 | `+` concat instead of f-string (mixed) | `"hi " + name` and `f"hi {name}"` | 🟢 |

### Repo level (multi-file)

| ID | Signal | Example | Tier |
|----|--------|---------|------|
| H50 | Human file names | `test2.py`, `Copy of x`, `final_v3.py`, `.bak` | 🟡 (update imports if renaming a module) |
...
2. **Branch on language.** Build the safe set from the LANGUAGE BRANCH table. In Python, drop
   all indentation/tab signals to 🔴.
...
5. **Apply 🟢 and verified-🟡 edits only.** Collect every 🔴 as a suggestion list.
6. **Verify** it still parses / tests still pass. Revert anything that breaks.
7. **Deliver:** the rewritten code, the list of signals injected (with IDs), the **🔴 flag-only
...
## WORKED EXAMPLE — brace language (lots of 🟢 room)
...
```
Behavior identical (sums positives). All edits are 🟢/🟡: brace-spacing (H10), operator
...
(verified 🟡 H30). **Flag-only (🔴) not applied:** none needed here.

## WORKED EXAMPLE — Python (indentation is 🔴, so lean on comments/idioms)
...
def active_users(users):
    ...
    return out
```
Behavior identical. Edits: `range(len(...))` expansion (H46, 🟢), no-space `#` comment (H11),
a venting/uncertain aside (H16), a typo in a comment `recieve` (H22, 🟢), operator/format
entropy (H1), a commented-out old one-liner (H12). **Indentation left untouched** (🔴 in
Python). **Flag-only suggestions emitted, NOT applied:** "change `u.get(\"is_active\")` to
