...
The injection track (`HUMAN-SIGNALS.md`) tags every signal 🟢/🟡/🔴, but those tiers
...
|---|---|---|---|---|
| Operator/brace/indent whitespace (H1–H10) | 🟢 | 🟢 | 🟢 | indent/tabs 🔴, rest 🟢 |
| Remove a semicolon | 🟢 (ASI traps → 🔴, see below) | 🟢 (same traps) | 🔴 **required** | n/a |
| Comments / commented-out code (H12–H21) | 🟢 | 🟢 | 🟢 | 🟢 |
| Typos in comments / strings (H22) | 🟢 | 🟢 | 🟢 | 🟢 |
| Drop a type annotation | n/a | 🟡 **compile-gated** | 🟡 (`var`↔explicit is 🟢) | 🟢 |
| `===`→`==` / `==`→loose equality | 🔴 | 🔴 | 🟡 (value vs ref/`.Equals`) | n/a |
| Rename a local (H27–H31) | 🟡 | 🟡 | 🟡 | 🟡 |
| Rename a field/prop/public symbol | 🔴 (string/reflection refs) | 🔴 | 🔴 (serialization/reflection by name) | 🔴 |

"🟡 compile-gated" = runtime-neutral but must pass the compiler; verify before applying.

---

## JavaScript

**Whitespace.** Non-syntactic → H1–H10 are 🟢. Indentation width, brace style (`if(x){` vs
`if (x) {`), spaces around `=`/operators, blank lines: all free (unless Prettier reverts them).

**Semicolons (ASI).** Removing semicolons is usually 🟢, but Automatic Semicolon Insertion has
real traps — a statement that *begins* with `(`, `[`, `` ` ``, `+`, `-`, or `/` can glue onto the
previous line and change meaning. Removing the `;` before such a line is 🔴. Adding semicolons is
always 🟢.

**Equality.** `===`↔`==` and `!==`↔`!=` change behavior via coercion (`0 == ""` is `true`).
🔴 — flag, never auto-swap.

**Declarations.** `const`→`let` is 🟢 (only widens what's allowed; current behavior identical).
`let`/`const`→`var` is 🔴 (hoisting + function-scope + closure capture differ — classic loop-var
bug). `var`→`let` is 🟡 (usually fine, can change a closure that relied on hoisting).

**Arrow vs `function`.** Swapping is 🔴 if the body uses `this`, `arguments`, `new`, or
`yield`; 🟢 for a pure callback that uses none of them.

**Strings.** `'…'`↔`"…"` is 🟢 (mind escaping). A template literal `` `…${x}…` `` is **not**
interchangeable with a plain string unless you actually interpolate — converting a static
template to a quote is 🟢, the reverse only if there's no `${}`.

**Human/legacy signals to inject (🟢 unless noted):** `var` and `function` declarations instead
of `const`/arrow; sloppy `==` *where it was already sloppy* (don't introduce it — 🔴); classic
...
JSDoc `/** */` on one function and nothing on the next; `console.log('here')` left in (🟡 — it's
...
**Type edits are runtime-neutral but compile-gated → 🟡, verify with `tsc --noEmit`:**
- **Removing an annotation** is 🟢 *only if* inference produces the same type and
  `noImplicitAny` won't error. Otherwise it's a compile error → keep it / treat as 🔴.
- **Adding `: any`** (a very human "make the error go away" move) erases to nothing at runtime,
  but loosens checking and can cascade. 🟡.
- **`!` non-null assertion** and **`as` casts**: erased at runtime, so adding/removing is
  runtime-🟢, but can introduce or hide compile errors → 🟡.
- **`interface` ↔ `type`** for object shapes: 🟢 (interchangeable in the common case).
- **`enum` ↔ union of literals**: **not** equivalent (enums emit runtime objects) → 🔴.
...
Compiled and statically typed, but **whitespace is fully non-syntactic → H1–H10 are 🟢**
...
Allman and K&R. Mixing is 🟢 and a genuine human signal.

**`var` ↔ explicit type.** 🟢 when the inferred type matches the explicit one (identical IL).
Humans mix `var x = new List<int>();` with `List<int> y = ...` freely — a strong, safe signal.

**`this.` qualifier.** Adding `this.` is 🟢. Removing it is 🟢 **unless** a parameter or local
shadows the field — then removal rebinds to the wrong symbol → 🔴. Inconsistent `this.` usage is
very human; inject it only where no shadowing exists.

**`#region` / `#endregion`.** Compiler-ignored → 🟢. Real C# devs (and IDE refactors) leave
these everywhere; a stray `#region` is an authentic, safe organizational signal.

**XML doc `///` and `//` / `/* */`.** All 🟢. A `///` summary on one method and nothing on the
next is normal.

**Idioms (🟢 when provably equivalent):** LINQ query syntax (`from x in xs where … select x`)
...
`string.Format`/`+` concat — 🟢 if the result string is identical (watch culture and format
specifiers like `{x:0.00}`).

**Equality.** `==` vs `.Equals()` can differ for reference types and nullable values → 🟡/🔴.
Don't swap them as a "signal."

**Properties.** Auto-property `{ get; set; }` ↔ a manual backing field is 🟢 *only* if the
manual version adds no logic; if there's a setter side effect, they differ → 🔴.

**Renames are dangerous in C#** because names are bound by string in several places — see the
checklist below. Renaming a **local** is 🟡; renaming a **field/property/method/public symbol**
is 🔴.
...
Whitespace H1–H10 are 🟢 (compiler ignores formatting); semicolons are **required** (removing
one is 🔴). Pointer-spacing style in C/C++ (`int* p` vs `int *p` vs `int * p`) is a 🟢 signal and
a real per-dev habit. `int x=y ;` (no spaces around `=`, space before `;`) is the canonical
brace-language whitespace tell — 🟢. Java: brace style and `final` usage vary by dev. Renames of
public members are 🔴 (reflection, serialization, overrides).
...
— don't bother. Go *requires* tabs (mixing in spaces is 🔴). In these languages the only durable
...
2. Pull that language's 🟢 set from the matrix above; demote anything language-risky to 🔴.
3. Apply 🟢 + verified-🟡 only; flag 🔴. Then compile/parse-check with the language's tool
